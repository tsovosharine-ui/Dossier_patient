...
import { creerPrescriptionEndoscopie } from '@/lib/prescriptionApi';
...
export default function EndoscopieForm({ patient, prescripteur }: Props) {
  ...
  async function handleSubmit() {
    ...
    setApiError("");
    try {
      await creerPrescriptionEndoscopie({ patientId: patient.id, prescripteurId: prescripteur.id, urgence, alertes, renseignements, typeExamen: typeEffectif, remarques });
        ...
        <div className="mb op" onClick={e => { if (e.target === e.currentTarget) setShowValidationModal(false); }}>
          <div className="mbox" style={{ maxWidth: 600, width: '95%', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ background: 'var(--navy)', color: '#fff', padding: '16px 20px', borderRadius: '20px 20px 0 0', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <h3 style={{ fontFamily: '"Manrope", sans-serif', fontSize: 18, fontWeight: 800, margin: 0 }}>Prescription endoscopie validée</h3>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ background: '#fff', border: '1px solid var(--bdr)', borderRadius: 10, padding: '12px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--navy-lt)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span className="ms" style={{ fontSize: 18, color: 'var(--navy)' }}>medical_information</span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--txt)', marginBottom: 4 }}>Type d&apos;examen</div>
                    <div style={{ fontSize: 12, color: 'var(--txt2)' }}>{validatedPrescription.typeExamen}</div>
                  </div>
                </div>
              </div>
