...
import { creerPrescriptionKine } from '@/lib/prescriptionApi';
...
export default function KineForm({ patient, prescripteur }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      await creerPrescriptionKine({ patientId: patient.id, prescripteurId: prescripteur.id, urgence, alertes, renseignements, typeKine: kineType === "Autre" ? kineAutre : kineType, diagnostic, contreIndications: [...ciSelected, ciAutre ? ciAutreText : undefined].filter(Boolean), objectifs, remarques });
        ...
        <div className="mb op" onClick={e => { if (e.target === e.currentTarget) setShowValidationModal(false); }}>
          <div className="mbox" style={{ maxWidth: 600, width: '95%', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ background: 'var(--navy)', color: '#fff', padding: '16px 20px', borderRadius: '20px 20px 0 0', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <h3 style={{ fontFamily: '"Manrope", sans-serif', fontSize: 18, fontWeight: 800, margin: 0 }}>Prescription kiné validée</h3>
            ...
            <div style={{ padding: '20px' }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--txt3)', marginBottom: 12 }}>Détails de la prescription</div>
                ...
                <div style={{ background: '#fff', border: '1px solid var(--bdr)', borderRadius: 10, padding: '12px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--navy-lt)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span className="ms" style={{ fontSize: 18, color: 'var(--navy)' }}>accessibility_new</span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--txt)', marginBottom: 4 }}>Type de kinésithérapie</div>
                    <div style={{ fontSize: 12, color: 'var(--txt2)' }}>{validatedPrescription.kineType === 'Autre' ? validatedPrescription.kineAutre : validatedPrescription.kineType}</div>
                  </div>
                </div>
                <div style={{ background: '#fff', border: '1px solid var(--bdr)', borderRadius: 10, padding: '12px 14px', marginBottom: 8 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--txt)', marginBottom: 4 }}>Diagnostic</div>
                  <div style={{ fontSize: 12, color: 'var(--txt2)' }}>{validatedPrescription.diagnostic}</div>
                </div>
                <div style={{ background: '#fff', border: '1px solid var(--bdr)', borderRadius: 10, padding: '12px 14px', marginBottom: 8 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--txt)', marginBottom: 4 }}>Objectifs thérapeutiques</div>
                  <div style={{ fontSize: 12, color: 'var(--txt2)' }}>{validatedPrescription.objectifs}</div>
                </div>
                {(validatedPrescription.ciSelected.length > 0 || validatedPrescription.ciAutre) && (
                  <div style={{ background: '#fff', border: '1px solid var(--bdr)', borderRadius: 10, padding: '12px 14px', marginBottom: 8 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--txt)', marginBottom: 4 }}>Contre-indications</div>
                    <div style={{ fontSize: 12, color: 'var(--txt2)' }}>{[...validatedPrescription.ciSelected, validatedPrescription.ciAutre ? validatedPrescription.ciAutreText : null].filter(Boolean).join(', ')}</div>
                  </div>
                )}
              </div>
